      SUBROUTINE besslz(y,lmin,lmax,fi,gi)
C   ******************************************************************
C   *                                                                *
...
C   ******************************************************************
      USE bessl_fact
      IMPLICIT NONE
      INTEGER :: lp1, nf, tlp1, ll1, ll2
      INTEGER :: lmin, lmax, i, isn, j1,j 2, k, l, lmx, lmxp1, lmxp2
      REAL(KIND=8)    :: dt2
      COMPLEX(KIND=8), PARAMETER ::  one = (1.d0,0.d0)
      COMPLEX(KIND=8) :: y, my, t, dt, exppr, srmy
      COMPLEX(KIND=8), DIMENSION(:), ALLOCATABLE :: dum
      COMPLEX(KIND=8), DIMENSION(lmin:*)         :: fi, gi
C
...
C
      IF(ABS(y).LT.tol) THEN
        DO 10 l = lmin, lmax
          fi(l) = CMPLX(1.d0/(4.d0*l+2.d0),0.d0)
   10     gi(l) = one
        RETURN
      ENDIF
C
