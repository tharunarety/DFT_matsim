      SUBROUTINE pathop(is,ga,bg0,gi,hgh,dtil,nzm)
C   ******************************************************************
C   *                                                                *
...
C   *                                                                *
C   *    3. Calculate the higher contribution to the spherical       *
C   *       density:                                                 *
C   *                                                                *
C   *           S(z) * g(z) * S(z)                                   *
C   *                                                                *
C   *       and integrate them over the 3D BZ.                       *
C   *                                                                *
C   * Notation:                                                      *
C   *                                                                *
C   *    ga    ----->  g(z)                                          *
...
C   ******************************************************************
      ...
      INTEGER :: it, ita, ntait, icon, icont, convt, minit
...
C
      ALLOCATE(sa(nlmqt,nlmq,0:nder))
      ALLOCATE(sal(nlmq,nlmq,0:nder))
      IF(expan.EQ.'D') THEN
         ALLOCATE(sap(nlmqt,nlmq,0:nderp))
...
C
      ...
      IF(conv(lz).EQ.0) THEN
         ...
         hgh(lz,1:nq,is,lmax+1:lmaxt)=zero
      ENDIF
   11 CONTINUE
C
      DO 20 lk=1,nkvec
...
C
      IF(stmp.EQ.'Y') THEN
C
C        Read the Bloch transformed slope matrices
C
         DO 21 i=1,nlmqt
         DO 21 j=1,nlmq
         READ(11) sa(i,j,0:nder)
         IF(expan.EQ.'D') READ(11) sap(i,j,0:nderp)
   21    CONTINUE
      ELSE
C
...
C
      IF(clkw0(lz).EQ.1) THEN
         CALL hghasa(is,hgh,sa,nder,lz,nzm,weight)
      ELSE
         CALL hghasa(is,hgh,sap,nderp,lz,nzm,weight)
      ENDIF
C
      gak=weight*gak
C
...
C
  ...
  120 FORMAT(/,' PATHOP: CPA equation for IS =',i3,
