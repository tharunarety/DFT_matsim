...
C   ******************************************************************
      ...
      REAL(KIND=8) :: aa, ba, ca
      INTEGER :: IPRIM, I, NQR2, K, J, IQ
  ...
  109 FORMAT(/,' PRIMV:**  Number of repeats NGHBP must be positive.')
...
C
     ...
     1          ' Construct lattice'
      READ(5,'(7x,i3,4(8X,I2))') NQ3,LAT,IPRIM,NGHBP,NQR2
      IF(NGHBP.LT.1) THEN
         WRITE(M6,109)
         STOP
      ENDIF
...
C
C        Take the first site as the origine
C
...
C
      ...
      aa=1.d0
C
      IF(ABS(ba-boa).GT.1.d-6) THEN
         WRITE(m6,*) 'WARNING: BA =',ba,' and BOA =',boa
         IF(msgl.ne.0) WRITE(msgio,*) 
     .               'WARNING: BA =',ba,' and BOA =',boa
      ENDIF
      IF(ABS(ca-coa).GT.1.d-6) THEN
         WRITE(m6,*) 'WARNING: CA =',ca,' and COA =',coa
         IF(msgl.ne.0) WRITE(msgio,*) 
     .               'WARNING: CA =',ca,' and COA =',coa
      ENDIF
C
      RETURN
C
