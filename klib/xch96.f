      SUBROUTINE exchlag(rho,s,u,v,pot,ex,vx)
C *********************************************************************
C *                                                                   *
C *   Local Airy Gas approximation for the exchange energy.           *
C *   INPUT:                                                          *
C *         rho : density                                             *
C *         s   : ABS(GRAD rho)/(2*kf*rho), kf=(3 pi^2 rho)^(1/3)     *
C *         u   : (GRAD rho)*GRAD(ABS(GRAD rho))/(rho**2 * (2*kf)**3) *
C *         v   : (LAPLACIAN rho)/(rho*(2*kf)**2)                     *
C *                                                                   *
C *   OUTPUT:                                                         *
C *         ex  : exchange energy per electron                        *
C *         vx  : exchange potential.                                 *
C *                                        L.Vitos on 20.09.1999.     *
C *********************************************************************
      IMPLICIT NONE
      REAL(KIND=8) :: rho, s, u, v, ex, vx
C
      REAL(KIND=8), PARAMETER :: thrd  = 1.d0/3.d0
      REAL(KIND=8), PARAMETER :: thrd4 = 4.d0/3.d0
      REAL(KIND=8), PARAMETER :: ax = -0.738558766382d0
      REAL(KIND=8), PARAMETER :: a1 = 0.041106d0, a2 = 0.092070d0
      REAL(KIND=8), PARAMETER :: a3 = 0.657946d0, a4 = 2.626712d0
      INTEGER :: pot
C
      REAL(KIND=8) :: exunif, fxlag, fs, fss, s4
      REAL(KIND=8) :: xs, zs, ys, xsd, xsdd, ysd, ysdd, zsd, zsdd
C
C     LDA exchange energy density
C
      exunif=ax*(rho**thrd)
C
C     LAG enhancement factor
C
      s4=s**a4
      xs=a1*s4
      zs=1.d0+a2*s4
...
C 
      ...
      fs=fs/s
      fss=xsdd/ys-2.d0*xsd*ysd/ys/ys+
     .     2.d0*xs*ysd*ysd/ys/ys/ys-xs*ysdd/ys/ys
      fss=(fss-fs)/s
...
c----------------------------------------------------------------------
      ...
      parameter(pi=3.14159265358979323846264338327950d0)
      parameter(ax=-0.738558766382022405884230032680836d0)
      parameter(um=0.1234567890123456d0,uk=0.8040d0,ul=um/uk)
c----------------------------------------------------------------------
c----------------------------------------------------------------------
c construct LDA exchange energy density
      exunif = AX*rho**THRD
