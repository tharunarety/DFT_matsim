      SUBROUTINE bessh(y,lmin,lmax,fi,gi)
C   ******************************************************************
C   *                                                                *
C   *   Calculate spherical Bessel functions j,                      *
C   *             spherical Neumann functions n,                     *
C   *             spherical Hankel functions n.                      *
C   *                                                                *
C   *  *On entry:                                                    *
C   *                                                                *
...
C   *                 i*h_l(x)*x^(l+1)/(2l-1)!! ; l=lmin,.,lmax.     *
C   *                                                                *
...
C   ******************************************************************
      ...
      REAL(KIND=8) :: y, dt, dt2, my, t, srmy, exppr
...
C  
      ...
      ENDDO
C
C     spherical Hankel functions if y < 0
C
      IF(y.LT.0d0) THEN
        srmy=DSQRT(-y)
        gi(0)=1.d0
        ...
        gi(lmin:lmax)=gi(lmin:lmax)*exppr
      ENDIF
C
C     Normalize Bessel and Neumann/Hankel functions
C           
      DO l=lmin,lmax
